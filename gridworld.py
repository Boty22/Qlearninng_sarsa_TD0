...
def parseOptions():
    ...
    optParser.add_option('-l', '--learningRate',action='store',
                         type='float',dest='learningRate',default=0.9,#too low
    ...
    #MODIFICATION LUCIA
    #if opts.manual and (opts.agent != 'q' or opts.agent != 'sarsa'):
    if opts.manual: 
        if opts.agent != 'q' and opts.agent != 'sarsa':
...
if __name__ == '__main__':
    ...
    ###########################
        ...
        gridWorldEnv = GridworldEnvironment(mdp)
        actionFn = lambda state: mdp.getPossibleActions(state)
        sarsaLearnOpts = {'gamma': opts.discount,
                      'alpha': opts.learningRate,
                      'epsilon': opts.epsilon,
                      'actionFn': actionFn}
